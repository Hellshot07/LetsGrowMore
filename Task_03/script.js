...
// ----------------------function to fetch the image from url entered in form and display it on the page
...
// --------------function to validate the URL entered in the form
...
const data = getData();
if (validateURL(data)) {
const success = await loadImage(data);
if (success) {
allUsersData.push(data);
printResult(data);
resetForm();
} else {
alert("Error: Unable to load image from the given URL");
}
} else {
alert("Error: Invalid URL format for image");
}
} else {
form.classList.add('was-validated');
};
